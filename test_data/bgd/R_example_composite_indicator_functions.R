...
compose_indicator_binarised_weighted <- function(data, variables, critical.values, weights){
  ...
  data_ci <- data[match(variables, names(data))] #take a subset of the data with only the columns needed for the composite indicator
  value_per_indicator <- mapply(recode.elements, data_ci, critical.values, weights) %>% as.data.frame
