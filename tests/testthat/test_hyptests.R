...
test_that("Hypothesis test limit works",{
  expect_error(hypothesis_test_t_one_sample(tf$numeric[2], 3,design = design)$results) ## too  categories for ChiSquared
  expect_equal(hypothesis_test_t_one_sample(tf$select_one_many_cat[2], limit = 3,design = design)$name, "too many (>=20) unique values in independent variable") ### NEEDS TO THROW THIS ISSUE BUT DOESNT
